...
public static class Optimizer
{
    ...
    /// <summary>
    /// Creates a study with Sobol quasi-random sampler.
    /// Provides better space-filling coverage than uniform random sampling.
    /// </summary>
    public static Study CreateStudyWithSobolSampler(
        string name,
        SearchSpace searchSpace,
        StudyDirection direction = StudyDirection.Minimize,
        SobolSamplerConfig? config = null,
        IPruner? pruner = null,
        IEnumerable<Trial>? warmStartTrials = null,
        Study? fromStudy = null)
    {
        var sampler = new SobolSampler(config);
        var study = new Study(name, sampler, searchSpace, direction, pruner);

...
public sealed class MedianPruner : IPruner
{
    ...
    public bool ShouldPrune(Trial trial, IReadOnlyList<Trial> trials)
    {
        ...
        // Skip warmup phase (use dimension-aware effective warmup steps)
        var effectiveWarmupSteps = _config.GetEffectiveNWarmupSteps();
        if (lastStep <= effectiveWarmupSteps)
            return false;
        ...
        foreach (var other in trials)
        {
            ...
            // Include both Complete and Pruned trials that have values at this step

...
public sealed class PopulationBasedTrainer
{
    ...
    public IReadOnlyList<PbtMember> AskPopulation()
    {
        ...
        {
            var parameters = sampler.Sample(new List<Trial>(), StudyDirection.Minimize, _searchSpace);
            var member = new PbtMember(
                Id: i,
                Parameters: parameters,
                Performance: double.NegativeInfinity,
                Step: 0,
                ParameterHistory: new List<Dictionary<string, object>> { new Dictionary<string, object>(parameters) }
            );

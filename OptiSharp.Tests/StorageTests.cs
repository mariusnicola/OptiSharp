...
public class StorageTests
{
    private SearchSpace CreateSimpleSpace() =>
        new SearchSpace(new ParameterRange[] { new FloatRange("x", 0, 10), new IntRange("y", 1, 5) });
    ...
    public void Storage_InvalidFile_Throws()
    {
        ...
        // Accept DirectoryNotFoundException on Windows (when parent path doesn't exist)
        // or FileNotFoundException on Unix (when file doesn't exist)
        var ex = Assert.Throws<System.IO.DirectoryNotFoundException>(
            () => Study.Load("/nonexistent/path.json", space, sampler));
        Assert.NotNull(ex);  // Verify exception was thrown
    }

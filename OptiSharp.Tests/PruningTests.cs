...
public class PruningTests
{
    private SearchSpace CreateSimpleSpace() =>
        new SearchSpace(new ParameterRange[] { new FloatRange("x", 0, 10) });
    ...
    public void NopPruner_NeverPrunes()
    {
        ...
        Assert.False(pruner.ShouldPrune(trial, new List<Trial> { trial }));
    }
    ...
    public void SuccessiveHalvingPruner_PrunesLowerRankedTrials()
    {
        ...
        // 5 completed background trials (required by MinTrialsBeforePruning bootstrap guard)
        var completedTrials = Enumerable.Range(0, 5)
            .Select(i =>
            {
                var t = new Trial(i, new Dictionary<string, object> { ["x"] = (double)i })
                {
        ...
        var bottom = new Trial(5, new Dictionary<string, object> { ["x"] = 5.0 });
        bottom.Report(2.5, 1);
    ...
    public void SuccessiveHalvingPruner_PrunesRunningTrial_WhenLowerRanked()
    {
        ...
        var worstRunning = new Trial(5, new Dictionary<string, object> { ["x"] = 5.0 });
        worstRunning.Report(2.5, 1);

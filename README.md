...
Both `MedianPruner` and `SuccessiveHalvingPruner` accept an `NDimensions` hint that adjusts aggressiveness to match the dimensionality of the problem. At low dimensions (p10), warmup steps are extended and the reduction factor is relaxed — preventing premature pruning on smooth, easy-to-navigate landscapes. At high dimensions (p100+), pruning is tightened to eliminate clearly bad trials quickly.

```csharp
var pruner = new MedianPruner(new MedianPrunerConfig { NDimensions = 10 });
var shaPruner = new SuccessiveHalvingPruner(new SuccessiveHalvingPrunerConfig { NDimensions = 100 });
```
...
**How it works:** Splits completed trials into "good" (best 10%) and "bad" (rest). Builds kernel density estimates (mixture of truncated Gaussians) for each group. Suggests parameters that maximize the Expected Improvement ratio l(x)/g(x). For high-dimensional problems, uses a multivariate KDE with Ledoit-Wolf shrinkage and Cholesky decomposition to capture parameter correlations. Based on [Bergstra et al., "Algorithms for Hyper-Parameter Optimization" (NeurIPS 2011)](https://papers.nips.cc/paper/2011/hash/86e8f7ab32cfd12577bc2619bc635690-Abstract.html), with bandwidth selection and magic clip following the Optuna implementation.

Dimension-adaptive behavior: the gamma threshold (fraction of "good" trials) and number of EI candidates scale with search space dimensionality so that the sampler remains effective at p50–p200.
...
**IPOP restarts:** When stagnation is detected (sigma collapses or improvement stalls), CMA-ES automatically restarts with doubled population size (IPOP strategy). This makes it robust on multimodal functions where a single run would get trapped in a local minimum.
...
The random sampler uses **quasi-random sequences** (Sobol for up to 50 dimensions, Halton for higher) to achieve better space-filling coverage than pure random, giving lower-discrepancy exploration for free.
...
The test suite covers correctness, convergence, and performance across **251+ tests**.

### Benchmark vs Optuna

Head-to-head comparison across 588 configurations (4 samplers × 4 dimensions × 4 trial budgets × 4 functions × 3 pruners):

| | OptiSharp | Optuna |
|---|---|---|
| **Overall win rate** | **77.7%** (457/588) | 22.3% (131/588) |
| CMA-ES | 91.7% | 8.3% |
| Random | 77.6% | 22.4% |
| TPE | 64.7% | 35.3% |

Tested on sphere, ackley, rosenbrock, and rastrigin functions at 10, 50, 100, and 200 dimensions with trial budgets from 100 to 100,000.
